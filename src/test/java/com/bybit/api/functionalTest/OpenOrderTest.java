...
public class OpenOrderTest {
    BybitApiTradeRestClient client = BybitApiClientFactory.newInstance("9poUAQLOWR5TaEpxcz", "q1pOcXWpiW4GTtYEeS26xx0zv7Az0T3qYP4z", BybitApiConfig.TESTNET_DOMAIN).newTradeRestClient();

    @Test
    public void Test_PlaceLinearOrder() throws InterruptedException {

        List<String> orderLinkIdList = new ArrayList<>();
        var newSpotOrderRequest = TradeOrderRequest.builder().category(CategoryType.SPOT).symbol("XRPUSDT").side(Side.BUY).orderType(TradeOrderType.MARKET).qty("15").timeInForce(TimeInForce.IOC);
        int lastIdx = 510;
        /*
